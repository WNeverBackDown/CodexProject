Create Codex pet row `running-right` for `mantou`: exactly 8 full-body frames in one horizontal strip on flat pure user-selected #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: A cute silver shaded round-faced house cat named Mantou, green eyes, pink nose, plush white and grey fur, calm obedient sleepy personality, often staring quietly, soft adorable sticker style.. Preserve silhouette, face, palette, material, proportions, markings, and props.
